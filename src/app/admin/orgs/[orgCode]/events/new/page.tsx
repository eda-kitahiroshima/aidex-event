...
export default function CreateEventPage({ params }: { params: Promise<{ orgCode: string }> }) {
  ...
  // 開始日時が変わったとき、終了日時が未設定 or 開始と同じなら自動補完
  const prevStartRef = { current: '' }
  useEffect(() => {
    if (startAt && (!endAt || endAt === prevStartRef.current)) {
      setValue('end_at', startAt)
    }
    prevStartRef.current = startAt
  ...
  const onSubmit = async (data: EventFormValues) => {
    ...
    try {
      ...
      const defaultBoards = [
        { event_id: eventData.id, board_type: 'all_notice',        title: '全体連絡',         description: '参加者・ボランティア全員への連絡事項です。', visibility: 'all',       post_permission: 'admin', reply_permission: 'none' },
        { event_id: eventData.id, board_type: 'staff',             title: 'スタッフ全体連絡', description: 'ボランティアスタッフ向けの全体連絡です。',   visibility: 'volunteer', post_permission: 'admin', reply_permission: 'volunteer' },
        { event_id: eventData.id, board_type: 'participant_notice', title: '参加者向けお知らせ', description: '参加者へのお知らせ事項です。',             visibility: 'all',       post_permission: 'admin', reply_permission: 'none' },
        { event_id: eventData.id, board_type: 'participant_qa',    title: '参加者Q&A',        description: '参加者からの質問と回答です。',              visibility: 'all',       post_permission: 'participant', reply_permission: 'admin' },
      ]
      ...
      if (data.has_survey) {
        ...
        await supabase.from('survey_questions').insert([
          { survey_id: surveyData.id, question_text: 'イベントは楽しかったですか？',   question_type: 'rating',  required: true,  sort_order: 1 },
          { survey_id: surveyData.id, question_text: '内容はわかりやすかったですか？', question_type: 'rating',  required: true,  sort_order: 2 },
          { survey_id: surveyData.id, question_text: 'また参加したいですか？',         question_type: 'yes_no', required: true,  sort_order: 3 },
          { survey_id: surveyData.id, question_text: '感想（自由記述）',               question_type: 'text',   required: false, sort_order: 4 },
        ])
      }
      ...
      console.error(error)
      setGlobalError((error as Error).message || 'イベントの作成に失敗しました')
    ...
    <div className="min-h-screen bg-gray-50 pb-12">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl shadow-sm p-6 space-y-6">
          {globalError && (
            <div className="p-4 bg-red-50 text-red-700 rounded-lg text-sm">{globalError}</div>
          )}
          ...
          <div className="space-y-5">
            ...
            {/* 日時（カスタムピッカー） */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 p-4 bg-blue-50 rounded-xl border border-blue-100">
              <DateTimePicker
                label="開始日時"
                required
                value={startAt}
            ...
            <div className="space-y-3 pt-4 border-t">
              <div className="flex items-center">
                <input type="checkbox" id="has_qr_checkin" {...register('has_qr_checkin')} className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500" />
                <label htmlFor="has_qr_checkin" className="ml-2 text-sm text-gray-700">当日会場でQR受付を行う</label>
              </div>
              <div className="flex items-center">
                <input type="checkbox" id="has_survey" {...register('has_survey')} className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500" />
                <label htmlFor="has_survey" className="ml-2 text-sm text-gray-700">イベント終了後にアンケートを取る</label>
              </div>
            </div>
          </div>

          <div className="pt-6 border-t flex justify-end gap-4">
            <Link href={`/admin/orgs/${orgCode}`} className="px-6 py-2 text-gray-600 hover:text-gray-900 font-medium">
              キャンセル
            </Link>
            <button
              type="submit"
